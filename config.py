"""Configuración compartida del pipeline: columnas, grupos categóricos, paths."""

from pathlib import Path

ROOT = Path(__file__).resolve().parent
PARQUET_DIR = ROOT / "Capturas_benignas"
PROCESSED_DIR = ROOT / "processed"
CHECKPOINT_DIR = ROOT / "checkpoints"

TIMESTAMP_COL = "bidirectional_first_seen_ms"

# Grupos de variables categóricas -> cada una tendrá su propia tabla de
# nn.Embedding, entrenada junto con el VAE. El agrupamiento es solo semántico
# (para razonar sobre el pipeline); cada columna se codifica individualmente.
CATEGORICAL_GROUPS = {
    "ip_mac": ["src_ip", "dst_ip", "src_mac", "dst_mac"],
    "port": ["src_port", "dst_port"],
    "l3l4_meta": ["protocol", "ip_version", "vlan_id", "tunnel_id", "expiration_id"],
    "app": [
...
CATEGORICAL_COLS = [c for group in CATEGORICAL_GROUPS.values() for c in group]

ALL_COLS = [
    "expiration_id",
    "src_ip", "src_mac", "src_port",
    "dst_ip", "dst_mac", "dst_port", "protocol", "ip_version",
...
NUMERIC_COLS = [c for c in ALL_COLS if c not in CATEGORICAL_COLS and c != TIMESTAMP_COL]

assert set(CATEGORICAL_COLS) | set(NUMERIC_COLS) | {TIMESTAMP_COL} == set(ALL_COLS)

READ_COLS = ALL_COLS  # columnas a leer de los parquet de nfstream

BATCH_SIZE = 200_000
TRAIN_FRACTION = 0.8

def embedding_dim_for_cardinality(n_categories: int) -> int:
    return round(max(100, n_categories) / 2)
